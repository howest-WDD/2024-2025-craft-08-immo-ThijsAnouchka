...
const showMarkers = function () {
  const arrLocations = document.querySelectorAll('.js-immo-location');
  for (const location of arrLocations) {
    const lat = location.dataset.lat;
    const lng = location.dataset.long;
    const marker = new mapboxgl.Marker().setLngLat([lng, lat]).addTo(mymap);
  }
};
